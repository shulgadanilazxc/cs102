...
class WriteTreeTestCase(TestCase):
    ...
    def test_write_tree_subdirs(self):
        ...
        alphabeta_tree_obj = (
            gitdir / "objects" / alphabeta_tree_sha[:2] / alphabeta_tree_sha[2:]
        )
        self.assertTrue(alphabeta_tree_obj.exists())

        numbers_tree_sha = "32ad3641a773ce34816dece1ce63cc24c8a514d0"
        numbers_tree_obj = (
            gitdir / "objects" / numbers_tree_sha[:2] / numbers_tree_sha[2:]
        )
        self.assertTrue(numbers_tree_obj.exists())


@unittest.skipIf(
    pyvcs.__version_info__ < (0, 6, 0), "cНужна версия пакета 0.6.0 и выше"
)
